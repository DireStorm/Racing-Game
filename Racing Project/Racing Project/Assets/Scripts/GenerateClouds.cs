...
public class GenerateClouds : MonoBehaviour
{
    ...
    private void BuildCloudBatch(int xLoop, int yLoop)
    {
        ...
        // Dimensions of cloud is determined by this loop
        for (int x = 0; x < 31; x++)
        {
            for (int y = 0; y < 31; y++)
            {
                //Creates Cloud each loop
    ...
    {
        foreach(var batche in batchesToUpdate)
        {
            Graphics.DrawMeshInstanced(cloudMesh, 0, cloudMat, batche.Select((a) => a.matrix).ToList());
        }
    }
}
